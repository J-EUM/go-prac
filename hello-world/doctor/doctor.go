package doctor // 패키지 폴더이름은 패키지이름과 같다(컨벤션)
...
var matches = []string{ // []string 스트링 슬라이스
	"life",
	"i need",
	"why don't",
	"why can't",
...
var reflections = map[string]string{ // 맵. 키-밸류 저장용
	"am":     "are",
	"was":    "were",
	"i":      "you",
	"i'd":    "you would",
...
var responses = [][]string{ // 스트링 슬라이스의 슬라이스...중간중간 %1은 뭘까용
...
Hello. How are you feeling today?` // 여러줄 스트링은 백틱 사용
}

// 함수에 주석을 달때 주석의 첫 단어를 함수이름으로 하는것이 컨벤션이다.
...
func Response(userInput string) string {
	...
	// of the input string.
	output = strings.ReplaceAll(output, "%1", remainder) // output에서 %1을 찾아서 for문에서 만든 remainder로 치환한다.
